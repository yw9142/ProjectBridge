...
2026-02-20 운영 정책 업데이트: `/api/projects`는 프로젝트 멤버십 기반 조회로 제한하고, 프로젝트 생성은 `PM_OWNER` 또는 `PLATFORM_ADMIN`만 허용한다. `/api/admin/tenants/{tenantId}/pm-users` 경로명은 유지하되 의미는 "테넌트 사용자"이며 기본 생성 역할은 `PM_MEMBER`다.
