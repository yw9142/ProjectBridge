...
public class PostController {
    ...
    public ApiSuccess<PostEntity> patch(@PathVariable UUID postId, @RequestBody PostPatchRequest request) {
        ...
        if (!principal.getUserId().equals(post.getCreatedBy()) && !isPmRole(member.getRole())) {
            throw new AppException(HttpStatus.FORBIDDEN, "POST_EDIT_FORBIDDEN", "Only author or PM can edit this post.");
        }
    ...
    public ApiSuccess<Map<String, Object>> delete(@PathVariable UUID postId) {
        ...
        ensureVisibleToMember(post, member);
        if (!principal.getUserId().equals(post.getCreatedBy()) && !isPmRole(member.getRole())) {
            throw new AppException(HttpStatus.FORBIDDEN, "POST_DELETE_FORBIDDEN", "Only author or PM can delete this post.");
        }
