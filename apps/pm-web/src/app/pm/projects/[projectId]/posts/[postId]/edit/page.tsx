...
export default function PostEditPage() {
  ...
  async function updatePost(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.replace(`/pm/projects/${projectId}/posts/${postId}`);
    } catch (e) {
      if (!handleAuthError(e, "/login")) {
  ...
  if (loading || !post) {
    return (
      <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <Link href={`/pm/projects/${projectId}/posts/${postId}`} className="text-sm text-indigo-600 hover:underline">
          상세로
        </Link>
  ...
  return (
    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <Link href={`/pm/projects/${projectId}/posts/${postId}`} className="rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 hover:bg-slate-100">
          상세로
        </Link>
      </div>
