...
export default function PmContractSigningStatusPage() {
  ...
  if (error) {
    return (
      <div className="space-y-3">
        <Button variant="outline" size="sm" onClick={() => router.push(`/admin/projects/${projectId}/contracts`)}>
          계약 목록으로
        </Button>
  ...
  return (
    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">서명 확인</h1>
          <p className="text-sm text-slate-500">계약의 서명 상태와 서명 반영 PDF를 확인합니다.</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => router.push(`/admin/projects/${projectId}/contracts`)}>
          계약 목록으로
        </Button>
      ...
      {signer?.assigned ? (
        <div className="rounded-lg border border-slate-200 p-3 text-sm">
          ...
          <p className="mt-1 text-slate-600">
            서명 요청: {formatEnvelopeStatus(signer.envelopeStatus)} / 수신 상태: {formatRecipientStatus(signer.recipientStatus)}
          </p>
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">지정된 서명자가 없습니다.</div>
      )}
